...
import './RegimenAfiliacionPage.css'; 
import RegimenAfiliacionDialogo from './RegimenAfiliacionDialogo.jsx';

const API_BASE_URL = import.meta.env.VITE_API_URL;
const ENDPOINT = `${API_BASE_URL}/admin/regimen-afiliacion`;

const RegimenAfiliacionPage = () => {
  ...
  const fetchDatos = async () => {
    try {
      ...
      console.error("Error cargando regímenes de afiliación:", error);
    } finally {
      setCargando(false);
    }
  };
  ...
  const handleToggleEstado = async (id) => {
    ...
    try {
      ...
      console.error("Error al cambiar estado del régimen:", error);
    }
  };
  ...
  const handleGuardar = async (datosFormulario) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(error.mensaje || "Error al procesar el régimen");
      }
    } catch (error) {
  ...
  return (
    <div className="admin-page-container">
      <header className="admin-page-header">
        <div>
          <h2 className="admin-title">Regímenes de Afiliación</h2>
...
export default RegimenAfiliacionPage;