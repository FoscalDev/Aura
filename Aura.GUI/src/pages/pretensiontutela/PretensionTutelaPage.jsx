...
import './PretensionTutelaPage.css'; 
import PretensionTutelaDialogo from './PretensionTutelaDialogo.jsx';

const API_BASE_URL = import.meta.env.VITE_API_URL;
const ENDPOINT = `${API_BASE_URL}/admin/pretension-tutela`;

const PretensionTutelaPage = () => {
  ...
  const fetchDatos = async () => {
    try {
      ...
      console.error("Error cargando pretensiones de tutela:", error);
    } finally {
      setCargando(false);
    }
  };
  ...
  const handleToggleEstado = async (id) => {
    ...
    try {
      ...
      console.error("Error al cambiar estado de la pretensión:", error);
    }
  };
  ...
  const handleGuardar = async (datosFormulario) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(error.mensaje || "Error al procesar la pretensión");
      }
    } catch (error) {
  ...
  return (
    <div className="admin-page-container">
      <header className="admin-page-header">
        <div>
          <h2 className="admin-title">Pretensiones de Tutela</h2>
        ...
        {cargando ? (
          ...
          <table className="aura-table">
            <thead>
              <tr>
                <th>Código</th>
                <th>Descripción de la Pretensión</th>
                <th>Estado</th>
...
export default PretensionTutelaPage;