...
import './CausaDemoraPage.css'; 
import CausaDemoraDialogo from './CausaDemoraDialogo.jsx';

const API_BASE_URL = import.meta.env.VITE_API_URL;
const ENDPOINT = `${API_BASE_URL}/admin/causa-demora`;

const CausaDemoraPage = () => {
  ...
  const fetchDatos = async () => {
    try {
      ...
      console.error("Error cargando causas de demora:", error);
    } finally {
      setCargando(false);
    }
  };
  ...
  const handleToggleEstado = async (id) => {
    ...
    try {
      ...
      console.error("Error al cambiar estado de la causa:", error);
    }
  };
  ...
  const handleGuardar = async (datosFormulario) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(error.mensaje || "Error al procesar la causa de demora");
      }
    } catch (error) {
  ...
  return (
    <div className="admin-page-container">
      <header className="admin-page-header">
        <div>
          <h2 className="admin-title">Causas de Demora</h2>
        ...
        {cargando ? (
          ...
          <table className="aura-table">
            <thead>
              <tr>
                <th>Código</th>
                <th>Descripción de la Demora</th>
                <th>Estado</th>
...
export default CausaDemoraPage;